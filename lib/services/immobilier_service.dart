...
class DatabaseService {
  ...
  Future<void> _createTables(Database db, int version) async {
    await db.execute(''' 
      CREATE TABLE immobilier (
        id_Imm INTEGER PRIMARY KEY AUTOINCREMENT,
        nom TEXT NOT NULL,
  ...
  Future<void> updateImmobilier(Immobilier immobilier) async {
    ...
    await db.update(
      ...
      immobilier.toMap(),
      where: 'id_Imm = ?',
      whereArgs: [immobilier.idImm], 
    );
  }

  Future<void> deleteImmobilier(int idImm) async {
    final db = await database;
    await db.delete('immobilier', where: 'id_Imm = ?', whereArgs: [idImm]); 
  }
}
